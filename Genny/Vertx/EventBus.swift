...
public class EventBus {
    ...
    func pingLoop() {
        ...
        let deadline = DispatchTime.now() + DispatchTimeInterval.milliseconds(self.pingInterval)
    ...
    public func send(to address: String,
        ...
        if let cb = callback {
            ...
            let deadline = DispatchTime.now() + DispatchTimeInterval.milliseconds(replyTimeout)
            DispatchQueue.global().asyncAfter(deadline: deadline, execute: timeoutAction)
        }
    ...
    public func unregister(address: String, id: String, headers: [String: String]? = nil) throws -> Bool {
        guard var handlers = self.handlers[address], let _ = handlers[id] else {
                return false
        }
